CREATE OR REPLACE PACKAGE BODY CDM.gdatasource2
/**
...
AS
   ...
   */
   ...
   BEGIN
      ...
      IF NOT g_debug
      THEN
		 --OWA_UTIL.mime_header ('text/x-json', FALSE, NULL); --original code
		 OWA_UTIL.mime_header ('application/json', FALSE, NULL); -- official IANA code see: http://www.iana.org/assignments/media-types/application/
         --OWA_UTIL.mime_header ('text/plain', FALSE, NULL);  -- compatible code
         HTP.p ('Pragma: no-cache,no-store');
         HTP.p ('Expires: -1');
