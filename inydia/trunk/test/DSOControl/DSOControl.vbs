...
Const strFilePath = "D:\My Dropbox\SOURCES\inydia\test\DSOControl\A1.mdb"
Const xmlFilePath = "file://D:\My Dropbox\SOURCES\inydia\test\DSOControl\A1.xml"
Const xmlRoot = "AssayResult"

Set adoConn = CreateObject("ADODB.Connection")
Set adoRS = CreateObject("ADODB.Recordset")
Call adoConn.Open("Provider=MSDAOSP; Data Source=MSXML2.DSOControl;")
Call adoRS.Open(xmlFilePath, adoConn)
Call adoRS.MoveFirst

Call Delete_File(strFilePath)
...
iLevel = 0
Call LoopThruFields (xmlRoot, iLevel, adoRS, db, Null)
iLevel = 0
Call LoopThruRela (xmlRoot, iLevel, adoRS, db, Null)
...
Sub LoopThruFields(ByVAl TblName, ByVal iLevel, ByVal adoRS, ByVal db, ByVal ForeignKey)
    iLevel = iLevel + 1
    PriorLevel = iLevel
    
    Set Tbl = db.CreateTableDef(TblName)
	Set Fld = Tbl.CreateField(TblName & "_ID", 4)
	Fld.attributes = 16
	Tbl.Fields.Append Fld
	Set ind = Tbl.CreateIndex("PK_" & TblName)
	With ind
	...
	If PriorLevel > 1 Then
		Set Fld = Tbl.CreateField(ForeignKey & "_ID", 4)
		'Fld.attributes = dbAutoIncrField
		Tbl.Fields.Append Fld
		Set ind = Tbl.CreateIndex("FK_" & ForeignKey)
		With ind
        ...
        For ndx = 0 To adoRS.Fields.Count - 1
            If adoRS.Fields(ndx).Name <> "$Text" Then
                If adoRS.Fields(ndx).Type = adChapter Then
		        	...
		        	ChildTable = adoRS.Fields(ndx).Name
                    Call LoopThruFields (ChildTable, iLevel, adoChildRS, db, TblName)
                Else
                    WScript.Echo iLevel & ": adoRS.Fields(" & ndx & ") = " & adoRS.Fields(ndx).Name & " = " & adoRS.Fields(ndx).Value & " : " & adoRS.Fields(ndx).Type
                    Set Fld = Tbl.CreateField(adoRS.Fields(ndx).Name, adoRS.Fields(ndx).Type)                   
                    Tbl.Fields.Append Fld
                End If
            End If
        Next 'ndx
...
Sub LoopThruRela(ByVAl TblName, ByVal iLevel, ByVal adoRS, ByVal db, ByVal ForeignKey)
    iLevel = iLevel + 1
    PriorLevel = iLevel
    
	If PriorLevel > 1 Then
		Dim rel 'As DAO.Relation
		Set rel = db.CreateRelation(ForeignKey & "_" & TblName)
		With rel
        ...
        For ndx = 0 To adoRS.Fields.Count - 1
            If adoRS.Fields(ndx).Name <> "$Text" Then
                If adoRS.Fields(ndx).Type = adChapter Then
                    Set adoChildRS = adoRS.Fields(ndx).Value
		        	ChildTable = adoRS.Fields(ndx).Name
                    Call LoopThruRela (ChildTable, iLevel, adoChildRS, db, TblName)
                End If
            End If
        Next 'ndx
    iLevel = PriorLevel
...
Dim objFSO 'As Object
       ...
       Call objFSO.DeleteFile(strFilePath)
       End If
       Set objFSO = Nothing
End Sub
...
Sub Check_File(strFilePath)
       ...
       Call Create_MDB_File(strFilePath)
       End If
       Set objFSO = Nothing
End Sub